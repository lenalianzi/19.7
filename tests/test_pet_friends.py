...
# Test 6
def test_add_pet_valid_data_no_photo(name='Хомяк_без_фото', animal_type='Хома', age='1'):
...
# Test 7
def test_add_photo_for_pet(pet_photo='images/Image 1.jpg'):
...
def test_delete_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Хомахрюш", "хомяк", "2", "images/Image 1.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
...
# Test 10
def test_create_pet_in_english(name='Cherry', animal_type='hamster', age=2):
    _, auth_key = pf.get_api_key(valid_email, valid_password)
